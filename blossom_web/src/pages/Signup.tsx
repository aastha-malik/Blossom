...
export default function Signup() {
  ...
  useEffect(() => {
    if (isAuthenticated) navigate('/today');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/verify-email', { state: { email, username, password } }), 500);
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Sign up failed. Please try again.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--rule)', padding: 32 }}>
          ...
          <p style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 14, fontStyle: 'italic', color: 'var(--ink-soft)', marginBottom: 28 }}>
            we will set you up with a fresh pet.
          </p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              {isLoading ? 'Creating account…' : 'Adopt your pet →'}
            </button>
          </form>
