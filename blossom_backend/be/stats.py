...
def get_user_tasks(db: Session, user_id: int, start_date: datetime):
    ...
    """
    tasks = db.query(Task).filter(Task.user_id == user_id).filter(Task.completed==True).filter(Task.created_at >= start_date).all()
    if tasks is None:
        raise HTTPException(status_code=404, detail="Tasks not found")
    else:
        return tasks

def count_tasks_completed(db: Session, user_id: int, start_date: datetime):
    """
    ...
    """
    num_completed = db.query(Task).filter(Task.user_id == user_id, Task.completed == True).filter(Task.created_at >= start_date).count()
...
def streak_calculation(db: Session, user_id: int, task: Task):
    """
...
def get_user_stats(db: Session, user_id: int, start_period_func):
    start_date = start_period_func()
    num_task_completed = count_tasks_completed(db, user_id, start_date)
    streaks_count = streak_calculation(db, user_id)
    xps = total_xps(db, user_id)
    focus_time = total_focus_time(db, user_id)
