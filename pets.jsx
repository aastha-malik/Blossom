// Tendr Pet Sprites
// 2 species × 4 life stages × 4 moods = 32 sprites
// Aesthetic: ink-drawn, slightly imperfect lines (Fraunces-era illustration),
// watercolor wash fills. Pairs with the cozy editorial brand.
//
// Each pet is parameterized — pass species, stage, mood, theme.
// Ink color, fill colors, and small features change with stage/mood.

// ─── shared palette (works on both light + dark via CSS vars upstream) ───
const INK = '#2a2118';          // pen line
const INK_SOFT = '#5a4d3c';
const ACCENT = '#c94e3a';       // tomato
const BLUSH = '#f4b6b0';        // cheek
const AMBER = '#c08a2a';
const LEAF = '#5b7a4c';

// ─── Mood mouth/eye drawers — return JSX <g> blocks ───
function eyes(mood, lEx, lEy, rEx, rEy, color = INK) {
  // mood: 'happy' | 'content' | 'sad' | 'sleepy'
  switch (mood) {
    case 'happy':
      // open round eyes with sparkle
      return (
        <g>
    ...
    case 'sad':
      // round eyes + tiny tear from one eye
      return (
        <g>
...
function mouth(mood, mx, my, w = 6, color = INK) {
  switch (mood) {
    case 'happy':
      // open smile
    ...
    case 'sad':
      ...
      return <path d={`M${mx - w / 2} ${my + 2} Q${mx} ${my - 1} ${mx + w / 2} ${my + 2}`}
                  fill="none" stroke={color} strokeWidth="1.4" strokeLinecap="round" />;
    case 'sleepy':
      // tiny zzz mouth (open, oval)
      return (
        <g>
          <ellipse cx={mx} cy={my + 1} rx="1.5" ry="2" fill={color} opacity="0.7" />
        </g>
      );
  }
}

function moodAura(mood, cx, cy, r) {
  // floats: hearts when happy, zzz when sleepy, droplet when sad, sparkle when content
  switch (mood) {
    case 'happy':
      return (
    ...
    case 'sad':
      return null; // tear is on the eye
    case 'content':
      return (
...
// DOG — 4 stages
// =====================================================================
function DogBaby({ mood, size = 110 }) {
  // tiny round puppy, oversized head, sleeping/wobbling
  const fur = '#d9a26a';
  const furDark = '#a87642';
  return (
...
function DogKid({ mood, size = 110 }) {
  // sitting puppy, head still big-ish, ears more defined
  const fur = '#c98e54';
  const furDark = '#8e5e30';
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
...
function DogTeen({ mood, size = 110 }) {
  // Golden retriever teen — facing front, fluffy floppy ears framing face
  const fur = '#e0b878';
  const furDark = '#b8894a';
  const furLight = '#f0d4a0';
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      {/* body — sitting front-facing, slightly fluffy chest */}
      <path d="M28 76 Q22 56 36 50 Q50 46 64 50 Q78 56 72 76 Q66 82 50 82 Q34 82 28 76 Z"
      ...
      {/* head — front-facing, rounded retriever shape */}
      <ellipse cx="50" cy="34" rx="19" ry="18" fill={fur} stroke={INK} strokeWidth="1.6" />
      {/* floppy ears framing the face — both down, golden retriever style */}
      <path d="M32 26 Q22 32 24 46 Q28 52 34 50 Q34 38 36 30 Z"
            fill={furDark} stroke={INK} strokeWidth="1.4" />
      <path d="M68 26 Q78 32 76 46 Q72 52 66 50 Q66 38 64 30 Z"
            fill={furDark} stroke={INK} strokeWidth="1.4" />
      {/* ear inner shadow */}
...
function DogAdult({ mood, size = 110 }) {
  // dignified, full grown, sitting tall
  const fur = '#9a663a';
  const furDark = '#5e3d20';
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      {/* body */}
      <path d="M28 70 Q22 48 40 40 L60 40 Q78 48 72 70 Q66 80 50 80 Q34 80 28 70 Z"
            fill={fur} stroke={INK} strokeWidth="1.6" />
...
// CAT — 4 stages
// =====================================================================
function CatBaby({ mood, size = 110 }) {
  // tiny kitten, curled
  const fur = '#e8d5b8';
  const stripe = '#a87642';
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      {/* triangular ears — sit ON TOP of head, point straight up */}
      ...
      {/* head, big — drawn AFTER ears so head covers ear bases cleanly */}
      <circle cx="50" cy="42" r="22" fill={fur} stroke={INK} strokeWidth="1.6" />
      {/* inner ear pink — smaller triangle inside, drawn over head edge */}
...
function CatKid({ mood, size = 110 }) {
  // sitting kitten, ears taller
  const fur = '#dcb37d';
  const stripe = '#8e5e30';
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      {/* ears — taller upward triangles on top of head */}
...
function CatTeen({ mood, size = 110 }) {
  // Maine Coon teen — front-facing, fluffy mane, big tufted ears wide apart on top
  const fur = '#c9924d';
  const furDark = '#7c5128';
  const furLight = '#e8c188';
  const stripe = '#7c5128';
  return (
    <svg width={size} height={size} viewBox="0 -8 100 108">
      <ellipse cx="50" cy="93" rx="34" ry="3.5" fill={INK} opacity="0.15" />
      {/* haunches/legs sitting */}
      ...
      {/* body — long fluffy maine coon */}
      <path d="M28 82 Q20 58 36 48 Q50 44 64 48 Q80 58 72 82 Q66 88 50 88 Q34 88 28 82 Z"
            fill={fur} stroke={INK} strokeWidth="1.6" />
      {/* mane fluff around chest */}
      ...
      {/* maine coon ears — wide apart, large upward triangles, drawn BEFORE head */}
      ...
      {/* head — front-facing, slightly squared, covers ear bases */}
      ...
      {/* forehead M-stripe (tabby marking) */}
      ...
      {/* whiskers — long, maine coon style */}
...
function CatAdult({ mood, size = 110 }) {
  // regal, sitting tall, tail wrapped at front
  const fur = '#a87236';
  const stripe = '#5e3d20';
  return (
    <svg width={size} height={size} viewBox="0 -8 100 108">
      ...
      {/* body — tall sitting */}
      <path d="M30 80 Q22 50 38 38 L62 38 Q78 50 70 80 Q66 88 50 88 Q34 88 30 80 Z"
            fill={fur} stroke={INK} strokeWidth="1.6" />
      ...
      {/* ears — tall, tufted upward triangles, drawn before head */}
      ...
      {/* mouth — cat-style M (always present base, mood overlays) */}
